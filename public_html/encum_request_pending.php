...
<body> 
  <div id="page">
            ...
            <p>Pending Encumbrance Requests</p>
					</div>
         </div>
				</div>
		  </header>   
...
$sql = "SELECT * FROM encum_req_pending";
...
if ($result->num_rows > 0) {
    ...
    echo '<form method="post">';
      echo "<tr><td>".$row["Date"].
            ...
            "</td><td>".$row["Line_Category"].
            "</td><td>".$row["Amount_Requested"].
            "</td><td>".$row["Check_Payable"].
            "</td><td>".$row["Check_ID"].
            "</td><td>".$row["Payment_Delivery"].
            "</td><td>".$row["Mail_To"].
            "</td><td>".$row["Request_Comments"].
            ...
            </td><td><a href='encum_request_view_pending.php?key=".$row["Form_ID"]."'class='button'>View</a></td></tr>";
	    echo '</form>';		
	}
  echo "</table>";
  } else {
...
if(isset($_POST["approve"]))
  {
    ...
    $sql2 = "SELECT * FROM encum_req_pending WHERE Form_ID = '$form_id'";
    ...
    $Email = $row2["Email"];
		...
		$Line_Category = $row2["Line_Category"];
		$Check_Payable = $row2["Check_Payable"];
    $Check_ID = $row2["Check_ID"];
		$Payment_Delivery = $row2["Payment_Delivery"];
		$Mail_To = $row2["Mail_To"];
    ...
    //Grabbing info to put into approved database
    $sql2 = ("INSERT INTO encum_req_approved(Date, First_Name, Last_Name, Student_ID, Organization, Email, Phone_Number, Line_Category, Amount_Requested, Check_Payable, Check_ID, Payment_Delivery, Mail_To, Request_Comments)
    VALUES('$Date', '$First_Name', '$Last_Name', '$Student_ID', '$Organization', '$Email', '$Phone_Number', '$Line_Category', '$Amount_Requested', '$Check_Payable', '$Check_ID', '$Payment_Delivery', '$Mail_To', '$Request_Comments')");

    if (mysqli_query($conn, $sql2)) {
      echo "<p>Form inserted to approved database successfully</p>";
      
      $sql2 = "DELETE FROM encum_req_pending WHERE Form_ID = '$form_id'";
